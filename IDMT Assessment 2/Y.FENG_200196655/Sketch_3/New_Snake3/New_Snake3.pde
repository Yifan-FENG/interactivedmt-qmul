...
void setup() { 
  ...
  myPort = new Serial(this, portName, 19200);  //19200 is serial speed 
}

//================================================ 
//new draw line for detecting input data 
void draw() {
  if ( myPort.available() > 0) {  // If data is available,
    val = myPort.read();         // read it and store it in val
    if (val<5)       //compare input value, if value range is 1-4, value represents directions 
      ked(val); 
    else      //if valye is out of range 1-4, value represents speed 
      speed = val/15;   //value/15 is to slow down the speed, orginal speed is set to 15 (int speed =15)
    println(val); //print is used to check on the input value  
  }
  //=========================================================
...
void gameover() {
  ...
  popMatrix();
  myPort.write(0);  //if game over, send a digit 0 to Arduino and Arduino will process motor event 
}
